...
final class LayoutService {
    private static let logger = Logger(subsystem: "com.ReDock.app", category: "LayoutService")
    ...
    func captureCurrentWindows(ignoreObstructed: Bool = false) -> [WindowSnapshot] {
        ...
        let obstructedIndices: Set<Int> = ignoreObstructed
            ? Self.obstructedWindowIndices(from: windows)
            : []

        return windows.enumerated().compactMap { index, window -> WindowSnapshot? in
            if ignoreObstructed && obstructedIndices.contains(index) {
                return nil
            }
    ...
    func captureWithObstructionInfo() -> (snapshots: [WindowSnapshot], obstructedIDs: Set<UUID>) {
        let windows = windowQuerying.allVisibleWindows()
        let obstructedIndices = Self.obstructedWindowIndices(from: windows)
        ...
        for (index, window) in windows.enumerated() {
            ...
            if obstructedIndices.contains(index) {
    ...
    static func obstructedWindowIndices(from windows: [WindowInfo]) -> Set<Int> {
        ...
        let orderedFrames: [(pid: pid_t, frame: CGRect)] = cgList.compactMap { dict in
            ...
            // Skip ReDock's own windows (settings, save dialog, etc.)
        ...
        // Build an ordered list of WindowInfo indices in Z-order
        var zOrderedAXIndices: [Int] = []
        ...
        // Tests and some edge cases may provide windows that cannot be resolved
        // back to a running application. Preserve the caller's front-to-back order
        // rather than disabling obstruction detection entirely.
        if zOrderedAXIndices.isEmpty {
            zOrderedAXIndices = Array(windows.indices)
        }

        // For each window, check if the union of all windows above fully covers it
        var obstructed = Set<Int>()
        for (i, index) in zOrderedAXIndices.enumerated() {
            let frame = windows[index].frame
            let coveringFrames = zOrderedAXIndices[0..<i].map { windows[$0].frame }
            if isFullyCovered(frame, by: coveringFrames) {
                obstructed.insert(index)
            }
        }
